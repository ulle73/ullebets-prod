...
def test_http_dispatch_exposes_matchup_evaluation_with_separate_denominators() -> None:
    database = MemoryDatabase(
        matchup_results=MemoryCollection([{"observation_key": "obs-1", "fixture_date_stockholm": "2026-08-22", "match_key": "m1", "stat_key": "cornerKicks", "period": "ALL", "scope": "total", "selected_direction": "over", "evidence_class": "forward", "valid_for_predictor": True, "lifecycle_status": "resolved_predictor_only", "predictor_verdict": "hit", "score": 80, "rank_position": 1, "signed_residual": 2.0, "valid_for_market": False}]),
    )
    ...
    assert payload["predictor"]["medianSignedResidual"] == 2.0
    assert payload["predictor"]["constantDirectionBaseline"]["bestDirection"] == "over"
    assert payload["predictor"]["scoreBuckets"][1]["label"] == "80–89,9"
    assert payload["predictor"]["scoreBuckets"][1]["resolved"] == 1
