...
from ullebets_v2.read_api.service import read_league, read_match_detail, read_team
...
def test_league_exposes_compact_stat_rows_for_all_current_team_contexts() -> None:
    ...
    database = MemoryDatabase(
        ...
        teamprofiles=MemoryCollection([home, away]),
        fixtures_canonical=MemoryCollection(),
    ...
    assert any(row == {
        'teamKey': 'team-home', 'teamName': 'Home FC', 'context': 'home', 'orientation': 'for',
        'statKey': 'fouls', 'period': 'ALL', 'value': 13.0, 'rank': 2, 'leagueAverage': 10.5,
    } for row in rows)
...
def test_match_detail_joins_forward_selections_and_results_without_recomputing_them() -> None:
    database = MemoryDatabase(
        ...
        matchups_score=MemoryCollection(), matchups_league_avg=MemoryCollection(), market_snapshots=MemoryCollection(),
        market_offers=MemoryCollection(), match_results_canonical=MemoryCollection(), match_stats_canonical=MemoryCollection(), teamprofiles=MemoryCollection(),
        forward_bets=MemoryCollection([{
            'selection_key': 's1', 'prediction_key': 'p1', 'match_key': 'm1', 'stat_key': 'fouls', 'period': 'ALL', 'scope': 'away',
            'direction': 'over', 'line_value': 12.5, 'saved_odds': 1.9, 'predicted_win_probability': 0.61, 'expected_roi_units': 0.159,
            'model_id': 'v6', 'model_status': 'forward_test_only', 'selection_policy_id': 'policy-1', 'valid_for_forward_evaluation': True,
        }]),
        forward_results=MemoryCollection([{
            'result_loop_key': 'r1', 'selection_key': 's1', 'prediction_key': 'p1', 'match_key': 'm1', 'stat_key': 'fouls', 'period': 'ALL',
            'scope': 'away', 'direction': 'over', 'line_value': 12.5, 'saved_odds': 1.9, 'settlement_status': 'settled', 'settlement_result': 'win',
            'actual_value': 14, 'win': True, 'pnl_units': 0.9, 'roi_units': 0.9, 'valid_for_performance': True, 'result_loop_status': 'settled',
