...
def persist_model_snapshot_records(
    database: Any,
    *,
    ...
    for row in model_snapshot_docs:
        result = database["model_snapshots"].update_one(
            {"selection_key": row["selection_key"]},
            {"$set": row},
            upsert=True,
        )
        model_snapshot_upserts += 1 if result.upserted_id is not None else 0
    ...
    return {
        "model_snapshot_upserts": model_snapshot_upserts,
