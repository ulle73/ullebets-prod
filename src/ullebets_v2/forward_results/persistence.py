...
from ullebets_v2.storage.collections import FORWARD_RESULTS


def persist_forward_result_records(
    database: Any,
    *,
    ...
    for row in result_docs:
        result = database[FORWARD_RESULTS].update_one(
            {"result_loop_key": row["result_loop_key"]},
            {"$set": row},
            upsert=True,
        )
        result_upserts += 1 if result.upserted_id is not None else 0
    ...
    return {
        "forward_result_upserts": result_upserts,
