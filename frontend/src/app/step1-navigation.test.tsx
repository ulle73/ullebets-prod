...
const dashboard = {
  ...
  generatedAt: '2026-08-12T20:30:00Z',
  ...
  selections: [
    {
      ...
      expectedRoiUnits: 0.14,
      modelId: 'v6',
      modelStatus: 'forward_test_only',
      policyId: 'policy-1',
...
const results = {
  summary: { rows: 1, settled: 1, wins: 1, losses: 0, pushes: 0, excluded: 0 },
  page: { limit: 50, offset: 0, hasMore: false },
  rows: [
    {
      resultLoopKey: 'r1',
...
function href(link: HTMLElement): string | null {
  return link.getAttribute('href');
}

describe('step 1 entity navigation', () => {
  it('renders a real league route backed by the league read endpoint', async () => {
    renderApp('/liga/league-a', {
      '/api/v1/dashboard': dashboard,
      '/api/v1/leagues/league-a': league,
    });

    expect(await screen.findByRole('heading', { name: 'League A' })).toBeInTheDocument();
    expect(href(screen.getByRole('link', { name: 'Home FC' }))).toBe('/lag/home');
    expect(href(screen.getByRole('link', { name: /Home FC.*Away FC/ }))).toBe('/matcher/m1');
  });

  it('shows a real not-found route instead of silently redirecting broken URLs', async () => {
  ...
  it('links matchup league, both teams and match to their entity routes', async () => {
  ...
  it('resolves saved watchlist match ids independently of the current dashboard date', async () => {
    ...
    renderApp('/watchlist', {
      ...
      '/api/v1/matches': {
        matches: [
          {
            ...dashboard.matches[0],
            matchKey: 'saved-match',
            homeTeamName: 'Saved Home',
            awayTeamName: 'Saved Away',
          },
        ],
      },
    });
  ...
  it('does not send a browser-derived date when no explicit dashboard date was selected', async () => {
