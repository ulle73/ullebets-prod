...
export function TeamLeagueComparisonChart({title,subtitle,accessibleName,rows}:Props){const hasComparableData=rows.some((row)=>row.deviationPct!==null);return <section className="team-comparison-chart" role="region" aria-label={accessibleName}>
 ...
 <div className="team-comparison-chart__plot" aria-hidden="true">
  {hasComparableData?<BarChart width={1100} height={300} data={rows} margin={{top:26,right:16,bottom:6,left:2}}><CartesianGrid stroke="var(--color-border)" vertical={false}/><XAxis dataKey="rowKey" tick={false} axisLine={false} tickLine={false}/><YAxis domain={domainFor(rows)} tickFormatter={(value)=>`${value>0?'+':''}${value}%`} width={48} tick={{fill:'var(--color-text-muted)',fontSize:10}} axisLine={false} tickLine={false}/><ReferenceLine y={0} stroke="var(--chart-average)" strokeWidth={2}/><Tooltip cursor={{fill:'rgba(255,255,255,.025)'}} formatter={tooltipValue} contentStyle={{background:'var(--color-card)',border:'1px solid var(--color-border)',borderRadius:8,fontSize:11}}/><Bar dataKey="deviationPct" maxBarSize={28} isAnimationActive={false}>{rows.map((row)=><Cell key={row.rowKey} fill={barColor(row.deviationPct)}/>) }<LabelList dataKey="deviationLabel" position="top" fill="var(--color-text-primary)" fontSize={9} fontWeight={700}/></Bar></BarChart>:<div className="team-comparison-chart__empty">Ligasnitt saknas för jämförelsen.</div>}
 </div>
