      ...
      {performance.isLoading ? (
        ...
        <>
          <section className="formula-performance-summary" aria-label="Sammanfattning av formeljämförelsen">
            ...
            <MetricTile label="P/L och ROI" value={percentage(performance.data.summary.roiPct)} detail={performance.data.summary.roiPct === null ? `${performance.data.summary.shadowBets - performance.data.summary.settledBets} öppna 1u-spel väntar på rättning` : `${units(performance.data.summary.pnlUnits)} på ${units(performance.data.summary.stakeUnits)}`} tone={performance.data.summary.roiPct === null ? 'neutral' : performance.data.summary.roiPct >= 0 ? 'good' : 'bad'} icon={<TrendingUp size={14} />} />
            <MetricTile label="Officiell CLV" value={percentage(performance.data.summary.averageClvPct)} detail={performance.data.summary.averageClvPct === null ? 'Väntar på officiell T-10-closing' : `${percentage(performance.data.summary.clvBeatRatePct)} slår closing · ${performance.data.summary.officialClvObservations} obs`} tone={performance.data.summary.averageClvPct === null ? 'neutral' : performance.data.summary.averageClvPct >= 0 ? 'good' : 'bad'} icon={<Target size={14} />} />
          </section>
