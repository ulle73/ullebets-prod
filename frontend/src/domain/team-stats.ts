...
export function strongestDeviations(team:RichTeamResponse,limit=4){const rows=allTeamInsights(team).filter((r)=>r.deviationPct!==null);return{above:[...rows].sort((a,b)=>(b.deviationPct??0)-(a.deviationPct??0)).slice(0,limit),below:[...rows].sort((a,b)=>(a.deviationPct??0)-(b.deviationPct??0)).slice(0,limit)};}
