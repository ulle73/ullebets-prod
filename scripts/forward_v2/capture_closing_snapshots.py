...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Capture V2 closing snapshots and derive canonical closing_lines rows.")
    ...
    parser.add_argument(
        "--refresh-derived",
        action="store_true",
        help="Refresh CLV tracking and forward results after closing lines are materialized.",
    )
...
def main() -> int:
    ...
    # Dry-runs still need V2 history so checkpoint selection matches a real run.
    database = read_database if read_database is not None else get_database(config)
    ...
    if args.refresh_derived:
        summary["derived_refresh"] = refresh_closing_dependents(
            database=database,
