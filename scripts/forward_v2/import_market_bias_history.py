...
def main() -> int:
    ...
    support={"teams": list(get_database(config)["support_teams"].find({})) if a.write else [], "leagues": list(get_database(config)["support_leagues"].find({})) if a.write else []}
    ...
    summary=run_market_bias_refresh(source_workflow="import_market_bias_history.py",source_kind="offline_v1_bootstrap",candidates=candidates,as_of=datetime.fromisoformat(a.as_of.replace("Z","+00:00")),profile_date=a.as_of[:10],database=get_database(config) if a.write else None,dry_run=not a.write); summary["bootstrap_audit"]=audit; a.report_path.parent.mkdir(parents=True,exist_ok=True); a.report_path.write_text(json.dumps(summary,default=str,indent=2)); print(json.dumps(summary,default=str)); return 0
