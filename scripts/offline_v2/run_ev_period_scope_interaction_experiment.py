...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Test regularized period and scope slope deviations in the "
            "shared nested temporal logistic model."
        )
    )
    parser.add_argument(
        "--market-frame",
...
def _candidate_frame(
    ...
    *,
    ...
    result = frame.copy()
    if variant in {"scope_deviations", "scope_period_deviations"}:
    ...
    if variant in {"period_deviations", "scope_period_deviations"}:
        result = add_categorical_interaction_features(
            result,
            category_column="period",
            source_columns=SOURCE_COLUMNS,
            deviation_values=("1ST", "2ND"),
        )
    return result
...
def main() -> int:
    ...
    for variant in variants:
        ...
        model_name = f"nested_logistic_{variant}"
