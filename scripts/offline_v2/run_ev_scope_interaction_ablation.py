...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ablate the V6 scope-interaction source groups under the "
            "same nested temporal procedure."
        )
    )
    parser.add_argument(
        "--market-frame",
    ...
    parser.add_argument(
        ...
        default=Path(
            "data/v2/ev_model/"
            "experiment_063_scope_feature_ablations"
        ),
    )
    parser.add_argument(
        "--evaluation-end-date",
...
def main() -> int:
    ...
    for profile, source_columns in ABLATION_PROFILES.items():
        candidate_frame = add_categorical_interaction_features(
            market_frame,
        ...
        model_name = f"scope_interaction_{profile}"
        ...
        variant_dir = args.output_dir / profile
