...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Test V6 scope interactions over fixed train-window and "
            "recency variants on identical outer test dates."
        )
    )
    parser.add_argument(
        "--market-frame",
...
def main() -> int:
    ...
    for train_days, half_life_days in TEMPORAL_VARIANTS:
        name = (
            f"scope_interaction_train{train_days}_"
            f"half{half_life_days:g}"
        )
